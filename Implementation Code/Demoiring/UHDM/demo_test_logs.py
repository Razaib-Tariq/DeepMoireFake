...
def test_model_fn(args, data, model, save_path, device):
    ...
    try:
        ...
        print(f"Padded Image Size: {in_img.size()}")
        ...
        with torch.no_grad():
            ...
            print(f"Output Image Size after Cropping: {out_1.size()}")
...
def img_pad(img, w_pad, h_pad, w_odd_pad, h_odd_pad):
    return F.pad(img, (w_pad, w_odd_pad, h_pad, h_odd_pad))
...
def process_subfolder(subfolder_path, category, save_prefix, device):
    ...
    for dataset_root in demo_datasets:
        ...
        for subfolder in os.listdir(dataset_root):
            ...
            if os.path.isdir(subfolder_path):
                print(f"Processing subfolder: {subfolder_path}")
                process_subfolder(subfolder_path, os.path.basename(dataset_root), save_prefix, device)
