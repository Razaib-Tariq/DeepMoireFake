...
def tensor2im(image_tensor, imtype=np.uint8):
    ...
    if image_numpy.ndim == 3:
        ...
        image_numpy = (np.transpose(image_numpy, (1, 2, 0)) * 255.0).clip(0, 255)
    elif image_numpy.ndim == 2:  # single channel grayscale
        image_numpy = (image_numpy * 255.0).clip(0, 255)
        image_numpy = np.tile(image_numpy[:, :, np.newaxis], (1, 1, 3))
    else:
        raise ValueError(f"Unsupported tensor dimension: {image_numpy.ndim}")
