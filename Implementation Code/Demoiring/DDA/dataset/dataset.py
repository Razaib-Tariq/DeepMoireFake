...
class CustomMoireDataset(Dataset):
    def __init__(self, moire_root, moire_type='Real', loader=default_loader):
        self.moire_root = moire_root
        self.moire_images = []

        # Real 폴더와 Fake 폴더의 모든 이미지를 로드합니다.
        for moire_type in ['Real', 'Fake']:
            type_root = os.path.join(moire_root, moire_type)
            if not os.path.exists(type_root):
                raise FileNotFoundError(f"Moire images root directory not found: {type_root}")
            
            for root, _, files in os.walk(type_root):
                for file in files:
                    if file.endswith(('.jpg', '.png')):
                        self.moire_images.append(os.path.join(root, file))

        if len(self.moire_images) == 0:
            raise ValueError("No moire images found in the dataset directories.")
        ...
        print(f"Loaded {len(self.moire_images)} moire images from {self.moire_root}")
    ...
    def __getitem__(self, index):
        ...
        label = moire_img_path  # Use the full path as the label
        return moire, label

class CustomMoireDataset_mouth(Dataset):
    def __init__(self, moire_root, loader=default_loader):
        self.moire_root = moire_root
        self.moire_images = []

        # Real/mouth 폴더와 Fake/mouth 폴더의 모든 이미지를 로드합니다.
        for moire_type in ['Real', 'Fake']:
            type_root = os.path.join(moire_root, moire_type, 'mouth')
            if not os.path.exists(type_root):
                raise FileNotFoundError(f"Moire images root directory not found: {type_root}")
